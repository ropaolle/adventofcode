...
const greatestFlow = (graph, src) => {
  ...
  while (queue.length > 0) {
    const [current, depth, flow] = queue.shift();
    if (!graph[current].open && flow - depth > maxFlow) {
      minutes++;
      maxFlow = flow - depth;
      g = current;
    }
    ...
    for (const neighbor of graph[current].valves) {
      if (!visited.has(neighbor)) {
        visited.add(neighbor);
        // queue.push([neighbor, depth + 1, maxFlow]);
        queue.push([neighbor, depth + 1, graph[neighbor].flow - depth]);
      }
    }
  }
  ...
  return [g, maxFlow, graph[g].flow, minutes];
};
...
const partOne = (input) => {
  ...
  greatestFlow(data, 'AA');
};

const partTwo = (input) => {
  const data = parse2(input);
  console.log(maxPressureReleased(data, 30));
...
function getShortestPath(valves, openable) {
  function findShortestPath(start) {
    ...
    while (unvisited.length > 0) {
      ...
      Object.keys(next.to).forEach((id) => unvisited.push([valves[id], steps + next.to[id]]));
    }
